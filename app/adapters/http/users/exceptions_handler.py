...
async def user_already_had_status_exception_handler(
    ...
    return JSONResponse(status_code=status.HTTP_409_CONFLICT, content=exc.message,)


async def user_not_found_exception_handler(
    request: Request, exc: UsersNotFoundError
) -> JSONResponse:
    return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=exc.message,)
